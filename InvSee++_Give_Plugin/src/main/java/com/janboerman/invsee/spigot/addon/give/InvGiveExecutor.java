...
class InvGiveExecutor implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (eitherStack.isLeft()) { sender.sendMessage("§l§8[§cx§8]§r §9Инвентарь §8» §f" + eitherStack.getLeft()); return true; }
                ...
                responseFuture.thenAcceptAsync(response -> {
                    if (response.isSuccess()) {
                        ...
                        if (map.isEmpty()) {
                            ...
                            if (plugin.getServer().getPlayer(uuid) == null)
                                ...
                                invseeApi.saveInventory(inventory).whenComplete((v, e) -> {
                                    if (e != null) plugin.getLogger().log(Level.SEVERE, "§l§8[§cx§8]§r §9Система §8» §fНе удалось сохранить инвентарь", e);
                                });
                            sender.sendMessage("§l§8[§a✔§8]§r §9Инвентарь §8» §fДобавлено §6" + originalItems + " §fв инвентарь игрока §7" + userName + "§f.");
                        } else {
                            ...
                            if (plugin.queueRemainingItems()) {
                                sender.sendMessage("§l§8[§e⚡§8]§r §9Инвентарь §8» §fНе удалось добавить предметы в инвентарь§8: §e" + finalItems + "§f, добавлено в очередь...");
                                queueManager.enqueueInventory(uuid, plugin.savePartialInventories() ? finalItems : originalItems);
                            } else {
                                sender.sendMessage("§l§8[§cx§8]§r §9Инвентарь §8» §fНе удалось добавить предметы§8: §e" + finalItems + "§f.");
                            }
                        ...
                        if (reason instanceof TargetDoesNotExist) {
                            ...
                            sender.sendMessage("§l§8[§cx§8]§r §9Инвентарь §8» §fПросмотр инвентаря §6оффлайн-игроков §fотключён.");
                        } else {
                            sender.sendMessage("§l§8[§cx§8]§r §9Инвентарь §8» §fНе удалось добавить предмет в инвентарь §7" + inputPlayer + " §fпо неизвестной причине.");
                        }
                    }
