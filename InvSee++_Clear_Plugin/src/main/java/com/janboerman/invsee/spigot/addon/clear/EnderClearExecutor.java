    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length >= 2) {
            ...
            if (eitherMaterial.isRight()) {
                ...
                sender.sendMessage("§c§l✖§r §9 §8» §f" + eitherMaterial.getLeft());
                return true;
            }
        }
                ...
                responseFuture.thenAcceptAsync(response -> {
                    if (response.isSuccess()) {
                        ...
                        if (finalItemType == null) {
                            inventory.clear();
                            sender.sendMessage("§a§l✔§r §9Инвентарь §8» §fЭндер-сундук игрока §7" + userName + " §6очищен§f.");
                        } else {
                            if (finalMaxCount == -1) {
                                finalItemType.removeAllFrom(inventory);
                                sender.sendMessage("§a§l✔§r §9Инвентарь §8» §fУдалены все предметы §6" + finalItemType + " §fиз §6эндер-сундука §7" + userName + "§f.");
                            } else {
                                int removed = finalItemType.removeAtMostFrom(inventory, finalMaxCount);
                                sender.sendMessage("§a§l✔§r §9Инвентарь §8» §fУдалено §6" + removed + " " + finalItemType + " §fиз §6эндер-сундука §7" + userName + "§f.");
                            }
                        }
                        ...
                        if (reason instanceof TargetDoesNotExist) {
                            ...
                            sender.sendMessage("§c§l✖§r §9Инвентарь §8» §fНе удалось очистить эндер-сундук §7" + inputPlayer + " §fпо неизвестной причине.");
                        }
                    }
