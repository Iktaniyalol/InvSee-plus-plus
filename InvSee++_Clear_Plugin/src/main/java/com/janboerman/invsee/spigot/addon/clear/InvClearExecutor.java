    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length >= 2) {
            ...
            if (eitherMaterial.isRight()) {
                ...
                sender.sendMessage("§8[§cx§8]§r §9Инвентарь §8» §f" + eitherMaterial.getLeft());
                return true;
            }
        }
                ...
                responseFuture.thenAcceptAsync(response -> {
                    if (response.isSuccess()) {
                        ...
                        if (finalItemType == null) {
                            inventory.clear();
                            sender.sendMessage("§8[§e✔§8]§r §9Инвентарь §8» §fИнвентарь игрока §7" + userName + " §6очищен§f.");
                        } else {
                            if (finalMaxCount == -1) {
                                finalItemType.removeAllFrom(inventory);
                                sender.sendMessage("§8[§e✔§8]§r §9Инвентарь §8» §fУдалены все предметы §e" + finalItemType + " §fиз инвентаря §7" + userName + "§f.");
                            } else {
                                int removed = finalItemType.removeAtMostFrom(inventory, finalMaxCount);
                                sender.sendMessage("§8[§e✔§8]§r §9Инвентарь §8» §fУдалено §e" + removed + " " + finalItemType + " §fиз инвентаря §7" + userName + "§f.");
                            }
                        }
                        api.saveInventory(inventory).whenComplete((v, e) -> {
                            if (e != null) plugin.getLogger().log(Level.SEVERE, "§8[§cx§8]§r §9Система §8» §fНе удалось сохранить §6инвентарь§f", e);
                        });
                    } else {
                        ...
                        if (reason instanceof TargetDoesNotExist) {
                            ...
                            sender.sendMessage("§8[§cx§8]§r §9Инвентарь §8» §fПросмотр инвентарей §6оффлайн-игроков §fотключён.");
                        } else {
                            sender.sendMessage("§8[§cx§8]§r §9Инвентарь §8» §fНе удалось очистить инвентарь §7" + inputPlayer + " §fпо неизвестной причине.");
                        }
                    }
